...
def cluster_predict(mfccs: np.ndarray, n_clusters=20) -> np.ndarray:
    scaler = StandardScaler()
    mfccs_scaled = scaler.fit_transform(mfccs.T)
    # get n_clusters based on the number of distinct people that introduce themselves
    kmeans = KMeans(n_clusters=n_clusters)  # Adjust based on the expected number of speakers
...
if __name__ == "__main__":
    ...
    groupedDf = df.value_counts(normalize=True).to_dict()
